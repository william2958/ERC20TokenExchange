...
contract('Buy Multiple Token Tests', function(accounts) {
	...
	it ("should be able to buy out some sell offers leave some of the last one remaining to sell", function() {
		...
		return exchange.deployed().then(function (instance) {
			...
			return exchangeInstance.buyToken("FIXED", web3.toWei(8, "finney"), 20);
		}).then(function(txResult) {
			assert.equal(txResult.logs.length, 1, "There should have been one Log Message emitted.");
			...
			assert.equal(orderBook[1].length, 0, "OrderBook should have 0 buy volume");
			return exchangeInstance.getSellOrderBook.call("FIXED");
		}).then(function(orderBook) {
			...
			assert.equal(orderBook[0][0], web3.toWei(3, "finney"), "Remaining sell price should be 3 finney");
			assert.equal(orderBook[1][0], 10, "There should be 10 tokens remaining to sell.");
